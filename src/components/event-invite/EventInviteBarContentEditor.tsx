...
export function EventInviteBarContentEditor({
  ...
  useEffect(() => {
    ...
    setStoryJson(formatStoryJson(initialStoryContent))
  ...
  const formInner = (
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      <label className="block space-y-1">
        <span className="text-[11px] text-zinc-400">Story JSON (CYOA passages, markdown in passage text)</span>
        <textarea
          required
          value={storyJson}
  ...
  return (
    <details className="mb-8 rounded-xl border border-amber-900/35 bg-amber-950/10 p-4 open:border-amber-800/50">
      <summary className="cursor-pointer text-sm font-semibold text-amber-200/90 list-none [&::-webkit-details-marker]:hidden">
        <span className="underline-offset-2 hover:underline">Edit headline &amp; story (stewards)</span>
        <span className="block text-[11px] font-normal text-zinc-500 mt-1">
          Title, subtitle, and CYOA JSON — same as Vault below.
        </span>
      </summary>
