...
export function AdminIdentitySwitcher() {
    ...
    useEffect(() => {
        ...
        getAllPlayersAdminPulse().then(setPlayers)
    }, [])

    if (players.length === 0) return null

    const handleSwitch = async (id: string) => {
        const result = await switchIdentityAdminPulse(id)
        if (result.success) {
            setIsOpen(false)
            ...
            {isOpen ? (
                <div className="bg-black border border-zinc-800 rounded-lg shadow-2xl p-2 w-64 max-h-96 overflow-y-auto mb-2">
                    <div className="flex justify-between items-center mb-2 px-2 pb-2 border-b border-zinc-900">
                        <span className="text-purple-500 font-bold">ADMIN SWITCHER</span>
                ...
                <button
                    ...
                    className="bg-purple-900/30 text-purple-400 border border-purple-900/50 hover:text-white hover:border-purple-500 w-10 h-10 flex items-center justify-center rounded-full shadow-lg transition-all"
                    title="Switch Identity (Admin Only)"
                >
                    🕵️
                </button>
            )}
