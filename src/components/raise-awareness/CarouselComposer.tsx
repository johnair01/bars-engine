...
export function CarouselComposer() {
  ...
  const updateSelectedText = (slideIndex: number, runIndex: number, patch: Partial<TextRun>) => {
    const run = post.slides[slideIndex].runs[runIndex]; const key = `${slideIndex}:${runIndex}`; const selection = textSelections.current[key]
    if (!selection || selection.start === selection.end) { editRun(slideIndex, runIndex, patch); return }
    const before = run.text.slice(0, selection.start); const selected = run.text.slice(selection.start, selection.end); const after = run.text.slice(selection.end)
    const replacements = [...(before ? [{ ...run, text: before }] : []), { ...run, text: selected, ...patch }, ...(after ? [{ ...run, text: after }] : [])]
    editSlide(slideIndex, { runs: [...post.slides[slideIndex].runs.slice(0, runIndex), ...replacements, ...post.slides[slideIndex].runs.slice(runIndex + 1)] })
    textSelections.current[key] = { start: 0, end: 0 }
  }
  const reset = () => { setPost(freshPost()); setActive(0); setNotice('Restored the six-slide starter draft.') }
