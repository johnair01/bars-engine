 ...
 * Monday with nothing to deploy. `serverNow` is the clock at request time; the
 * browser's takes over on mount, which stops a cached response naming
 * yesterday's day.
 *
 * A returning reader is offered their own next day instead of the beginning.
 */
export function ChallengeBanner({ serverNow }: { serverNow: number }) {
  const now = useClientClock() ?? serverNow
  const { progress, ready } = useCourseProgress()

  const latest = latestReleasedDay(now)
  ...
  const days = Array.from({ length: MTGOA_COURSE_LENGTH }, (_, i) => i + 1).map((number) => {
    ...
    return {
      ...
      gate: dayGate({
        ...
        dayReleased: isDayReleased(number, now),
        progress,
      }),
    }
  })
