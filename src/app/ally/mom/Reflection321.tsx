...
export function Reflection321({ initial, shared }: { initial: Reflection | null; shared: Synthesis[] }) {
  ...
  const save = () => startTransition(async () => { const result = await savePrivateReflection(values); setNotice(result.ok ? 'Saved privately. No one else in the room can read these entries.' : result.error) })
  const share = () => startTransition(async () => { const result = await publishReflectionSynthesis({ synthesis }); setNotice(result.ok ? 'Your selected synthesis is now shared. Your full reflection remains private.' : result.error) })
