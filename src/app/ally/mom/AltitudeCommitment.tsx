...
export function AltitudeCommitment({ commitments, participantId }: { commitments: Commitment[]; participantId: string }) {
  ...
  const save = () => startTransition(async () => { const result = await saveAltitudeCommitment({ altitude, handoffNote }); setNotice(result.ok ? 'Your role and handoff note are shared with the room.' : result.error ?? 'Unable to save your role.'); if (result.ok) router.refresh() })
