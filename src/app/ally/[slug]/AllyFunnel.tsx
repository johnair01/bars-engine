...
export function AllyFunnel({
  ...
  function finish() {
    ...
    if (testMode) {
      // Count across EVERY workstream, not just the one on screen. The checkout
      // step lets a reader collect picks from several areas, so scoring the dry
      // run off `offeredNeeds` would under-report the moment they took work from
      // more than one — and a rehearsal that quietly reports the wrong total is
      // worse than no rehearsal.
      const everyNeed = workstreams.flatMap((w) => w.needs)
      setResult({
        claimed: picked.size,
        skipped: [],
        vibeulons: everyNeed
  ...
  return (
    <div className="flex w-full flex-col gap-6">
          ...
          <Row>
            <button
              ...
              onClick={() => {
                ...
                else go(invite.showPlan ? 'plan' : 'domains-primer')
              }}
            >
      ...
      {/* ── plan (invite-gated) ───────────────────────────────────────────── */}
      {step === 'plan' && <PlanScreen onContinue={() => go('domains-primer')} />}

      {/* ── domains primer — teach the frame before asking them to use it ──── */}
      {step === 'domains-primer' && <DomainsPrimer onContinue={() => go('domain')} />}
