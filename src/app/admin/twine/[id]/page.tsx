    ...
    return (
        <div className="space-y-8">
                                ...
                                <div key={b.id} className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-3 flex items-center justify-between gap-3">
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${b.actionType === 'EMIT_QUEST' ? 'bg-blue-900/30 text-blue-400' : 'bg-green-900/30 text-green-400'}`}>
                                                {b.actionType}
                                            </span>
                                            <span className="text-xs text-zinc-500">on passage: <span className="text-zinc-300">{b.scopeId}</span></span>
                                        </div>
                                        <p className="text-white text-sm mt-1 truncate">{payload.title}</p>
                                    </div>
            ...
            <section className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                ...
                <BindingForm storyId={story.id} passageNames={passageNames} />
            </section>
        </div>
    )
}
