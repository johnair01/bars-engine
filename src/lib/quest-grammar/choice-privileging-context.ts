 ...
 *
 * ~100 tokens. Injected into buildQuestPromptContext when targetNationId/targetArchetypeId present.
 * CE: Nation and Archetype Choice Privileging.
 */
 ...
 * Style guide: 2–4 choices per passage. Privilege nation element + archetype WAVE.
...
export function buildChoicePrivilegingContext(
  ...
  const waveName = WAVE_NAMES[archetypeWave]
...
- Target archetype primary WAVE: ${waveName}. Offer at least one choice favoring a move whose primary WAVE matches (e.g. showUp → Step Through, Achieve Breakthrough, Declare Intention).
