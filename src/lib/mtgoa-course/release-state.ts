import { releasedRounds } from './course-release'

/**
 * The rounds open right now, resolved on the server.
 *
 ...
 *
 * Without this the server has no clock at all, and every week renders as though
 * it had yet to open — which put a past date on week 1 and served a board with
 * no day links to crawlers and to anyone without JavaScript.
 */
export async function currentReleasedRounds(): Promise<number[]> {
  return releasedRounds(Date.now())
}
