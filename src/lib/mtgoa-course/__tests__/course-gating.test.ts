...
describe('the release calendar', () => {
  ...
  it('treats week 1 as long live and gives days 16 to 30 no date at all', () => {
    for (const day of [1, 2, 3, 4, 5]) expect(isDayReleased(day, SATURDAY)).toBe(true)
    // Days 11–15 are the dated part of Week 3.
    ...
    expect(DAY_RELEASE_ISO[14]).toBe('2026-09-03T04:00:00Z')
    expect(DAY_RELEASE_ISO[15]).toBe('2026-09-04T04:00:00Z')
    for (const day of [16, 20, 30]) {
  ...
  it('names the next day still to come, and nothing once every dated day is live', () => {
    ...
    // Once Day 11 is live, Day 12 is the next dated day.
    expect(nextDayRelease(Date.parse('2026-08-30T05:00:00Z'))).toEqual({
      day: 12,
    ...
    // Once Day 13 is live, Day 14 is the next dated day.
    expect(nextDayRelease(Date.parse('2026-09-02T05:00:00Z'))).toEqual({
      day: 14,
      at: Date.parse('2026-09-03T04:00:00Z'),
    })
    // Once Day 14 is live, Day 15 is the next dated day.
    expect(nextDayRelease(Date.parse('2026-09-03T05:00:00Z'))).toEqual({
      day: 15,
      at: Date.parse('2026-09-04T04:00:00Z'),
    })
    // Once Day 15 is live, no dated day is left.
    expect(nextDayRelease(Date.parse('2026-09-04T05:00:00Z'))).toBeNull()
  })
