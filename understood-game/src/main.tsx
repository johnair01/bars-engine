...
const demoSteps = [
  { title: 'A disagreement, held on the board', body: 'Ari and Bea had a shared plan. Bea changed it without telling Ari. Guide both fictional readers; nobody has to type or perform a real disclosure.', cta: 'Begin the rehearsal' },
  { title: 'Ari names a boundary', body: 'Ari: “Please tell me before our plans change. I need to be included.” Place one red anger chip face up in Ari’s first stack.', cta: 'Place Ari’s anger chip' },
  { title: 'Bea asks rather than counters', body: 'Bea offers a neutral chip to ask about Ari’s open bid. Curiosity holds a chip while Ari answers.', cta: 'Bea asks: tell me more' },
  { title: 'Listen to the extra detail', body: 'Ari: “I made space for us, then learned the plan had changed. I felt left out.” Bea hears more, so the offered chip returns.', cta: null },
  { title: 'The conversation gets hotter', body: 'Ari notices their voice speeding up. Signaling heat gives both readers a visible reason to slow this moment down.', cta: 'Signal rising temperature' },
  ...
  { title: 'One chip, one effect', body: 'Choose resolution in the release dialog. The chip goes to Bea; heat does not change this time.', cta: null },
  { title: 'Return to Bea’s feeling', body: 'Bea’s green chip is still on the board. Select it. Both readers have a stake in how the plan gets repaired.', cta: 'Select Bea’s delight chip' },
  { title: 'Bea decides whether she was heard', body: 'Bea: “I want us to keep room for spontaneous invitations and still check in first.” Mark her chip heard.', cta: 'Bea says she feels heard' },
  { title: 'Bea moves toward resolution', body: 'Choose resolution for Bea’s chip. Her hidden meter falls while the public temperature remains settled.', cta: null },
  { title: 'The board made room and made progress', body: 'Heat rose and had time to cool. Both readers’ hidden conflict meters reached zero through heard chips. Replay, or begin your own two-person game.', cta: null },
] as const;
...
function demoGame(): Game { return { players: [{ name: 'Ari', meter: 1, hand: hand() }, { name: 'Bea', meter: 1, hand: hand() }], stacks: [emptyStacks(), emptyStacks()], turn: 0, played: false, placed: false, heat: 0, heatSignals: [false, false], turnDuration: 0, turnStartedAt: Date.now(), question: null, trade: null, history: ['Ari and Bea begin a guided conversation.'] }; }
...
function App() {
  ...
  const flip = () => { if (!choice) return; change(next => { const chip = next.stacks[choice.seat][choice.stack].find(item => item.id === choice.chipId); if (chip) chip.open = !chip.open; }); };
  const ask = (selected: Chip) => {
    if (!game || !choice || !selected.spoken || !selected.open || game.question || game.trade) return;
    ...
    setModal(null);
    setMessage('Ask aloud. The chip waits while your partner shares.');
  };
  const answer = (more: boolean) => {
    if (!game?.question) return;
    const q = game.question;
    if (q.offer) animateChip(q.offer, '#question-spot', `#rack-${more ? q.asker : q.speaker}-${q.offer}`);
    change(next => { if (q.offer) next.players[more ? q.asker : q.speaker].hand[q.offer]++; next.question = null; record(next, more ? 'More was shared; the offered chip returned.' : 'Nothing more was shared; the speaker kept the offered chip.'); });
    setMessage('The question is complete.');
    if (demoMode && demoStep === 3 && more) setDemoStep(4);
  };
  const donate = () => {
  ...
  const canAsk = !!selected && selected.spoken && selected.open && !game.question && !game.trade;
  ...
  const turnPanel = <div className={`board-turn-panel board-turn-panel-${game.turn}`} aria-label={`${game.players[game.turn].name}'s turn`}>
    ...
    <div className="board-turn-economy"><strong>ON YOUR TURN</strong><span>Place one chip, spoken or silent</span><strong>ANYTIME</strong><span>Ask about an open chip · donate · trade · release when heard</span></div>
  </div>;

  return <main className="live-page"><header className="live-header"><div className="brand">◌ <span>UNDERSTOOD</span></div><div className="header-actions"><span className="live-label"><i /> LIVE TABLE</span><button onClick={() => setModal('help')}>How to play</button>{demoMode ? <button onClick={leaveDemo}>Leave demo</button> : <button onClick={startDemo}>Guided demo</button>}<button onClick={reset}>{demoMode ? 'Replay demo' : 'New game'}</button></div></header><div className={`game-wrap${demoMode ? ' demo-mode' : ''}`}>{demoMode && <section className="demo-guide" aria-live="polite"><div className="demo-guide-top"><span className="eyebrow">GUIDED REHEARSAL · {Math.min(demoStep + 1, demoSteps.length)} / {demoSteps.length}</span><span>FICTIONAL READERS · ARI & BEA</span></div><h2>{demoSteps[demoStep].title}</h2><p>{demoSteps[demoStep].body}</p><div className="demo-guide-actions">{demoSteps[demoStep].cta && <button className="primary" onClick={runDemoStep}>{demoSteps[demoStep].cta} →</button>}{demoStep === 10 && <button onClick={() => setMessage('Ari is not ready to mark this chip heard. It stays in the stack.')}>Not yet heard</button>}{demoStep === 20 && <><button className="primary" onClick={leaveDemo}>Start or return to a real game →</button><button onClick={reset}>Replay the rehearsal</button></>}{![3, 14, 16, 19, 20].includes(demoStep) && <button className="demo-skip" onClick={leaveDemo}>Leave demo</button>}</div></section>}{message && <div className="message" role="status">{message}</div>}{won && <div className="win-message">Both players feel resolved. You can keep playing for the love of the game.</div>}
    ...
    {modal && <div className="overlay" onMouseDown={event => { if (event.target === event.currentTarget) setModal(null); }}><div className="dialog"><button className="dialog-close" aria-label="Close" onClick={() => setModal(null)}>×</button>{modal === 'release' && selected && choice ? <><span className="eyebrow">A FEELING WAS HEARD</span><h2>What changed?</h2><p>{game.players[choice.seat].name} chooses where the relief goes. The heard chip moves to the listener either way.</p><div className="release-options"><button disabled={game.heat === 0 || (demoMode && demoStep !== 14)} onClick={() => release(selected, 'cool')}><strong>Cool the conversation</strong><span>{game.heat === 0 ? 'Temperature is already settled' : <>Visible temperature {game.heat} → {game.heat - 1} / 5</>}</span></button><button disabled={game.players[choice.seat].meter === 0 || (demoMode && demoStep !== 16 && demoStep !== 19)} onClick={() => release(selected, 'resolve')}><strong>Resolve part of the issue</strong><span>Private conflict meter −1</span></button></div></> : modal === 'placed' && selected && choice ? <>
      ...
      <div className="draft-preview"><Token color={selected.color} open={selected.open} silent={!selected.spoken} heard={!!selected.heard} /><div><p>{selected.spoken ? moves[selected.color].prompt : 'This feeling is waiting to be spoken on a future turn.'}</p></div></div>
      <div className="draft-section"><span className="eyebrow">CHIP FACE</span><div className="draft-faces"><button className={selected.open ? 'selected-draft' : ''} onClick={() => { if (!selected.open) flip(); }}>? Face up <small>Questions welcome</small></button><button className={!selected.open ? 'selected-draft' : ''} onClick={() => { if (selected.open) flip(); }}>· Face down <small>Feeling only</small></button></div></div>
