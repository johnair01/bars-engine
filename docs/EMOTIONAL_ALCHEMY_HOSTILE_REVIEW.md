...
### Safety
- **S1 · Numb/dissociated players evade the crisis net entirely.** `B` — The intensity crisis check keys on a *number*. But the most at-risk players often **can't rate** — they tap "flat / numb" or "I can't tell," so `intensity` is `undefined` and no crisis logic ever runs. The flat-fork's `walled_off` = frozen grief behind a wall; that population is exactly who should see the outside-help path, and they're the one path that structurally can't. **Fix**: the flat-fork `walled_off` and `can't-tell` branches need their own gentle outside-help offer independent of a numeric rating.
- **S2 · Crisis resources are US-only, hardcoded.** `M` — `988` is baked into the crisis card and now the re-rate. Non-US players get a number they can't call. Gap **G10** flagged region-config; for a *safety* surface this is not cosmetic. **Fix**: a `crisisResources(region)` seam with a safe international fallback ("your local emergency number / findahelpline.com") before this ships to anyone outside the US.
- **S3 · No repeated-distress noticing.** `M` — The internal-only ratchet (§9.3) and thread demotion are *designed* but unbuilt, and there's no session log, so the system cannot notice a player who runs the diagnostic at 8+ every day. The one signal that most warrants "maybe more than a practice" — a *pattern* — is invisible. Ties to S-log being unbuilt (§Architect, §Sage).
- **S4 · Capture caps intensity at 5, below the crisis range.** `B` (latent, in the service design) — `/capture` collects `intensity: 1|2|3|4|5`. The service seeds the diagnostic from it. A captured charge can therefore **never** seed a 9–10, so the highest-distress captured charges silently drop below the crisis threshold when metabolized. **Fix**: normalize honestly and/or move capture to 0–10; never let the scale mismatch swallow a crisis signal.
