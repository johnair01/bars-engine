...
- `/understood/demo` opens with an interactive lesson for the five emotional chips: tap each color to see its move and flip a chip to learn what face up and face down communicate. It then runs a 21-step Wendell and Giuseppe scenario on the real game board. Visitor taps advance the same move functions used in live play; the guide shows spoken bids, a paid question, a public heat signal, a face-down silent second feeling that is later revealed, hearing every layer of a stack, cooling, private resolution, turn handoff, and both readers reaching zero.
...
- A player may offer a chip to ask about a face-down or unspoken chip at any time. If the owner reveals and answers, the chip turns face up and is marked spoken; the offered chip returns to the asker. The owner then gets an immediate **That answer resolved this chip** choice, which marks it heard. The stack still cannot release until all its layers are heard. The owner may instead decline, returning the bid without revealing, or say there is nothing more to tell and keep the bid. A manual face flip remains available only during the chip owner's turn.
