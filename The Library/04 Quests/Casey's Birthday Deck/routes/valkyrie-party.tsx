...
function Panel({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section style={{ background: PARTY_PANEL, border: `1px solid rgba(255, 176, 0, 0.28)`, borderRadius: 8, padding: "1rem", boxShadow: "0 0 28px rgba(255, 79, 163, 0.08)" }}>
      <h2 style={{ color: PARTY_GOLD, fontFamily: "Georgia, serif", fontSize: "1rem", letterSpacing: "0.08em", margin: "0 0 0.85rem" }}>
        {title}
      </h2>
      {children}
    </section>
...
function CardFace({
  ...
  const image = card.uploaded && card.image_file?.startsWith("/images/oracle/") ? card.image_file : null;
...
function CardBack({ onClick }: { onClick?: () => void }) {
  return (
    ...
    >
      <img src="/images/oracle/card-back.png" alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
    </button>
  );
}
...
export default function ValkyrieParty() {
  ...
  const uploadQuestPhoto = useCallback(async (file: File | null) => {
    ...
    try {
      ...
      body.set("file", file);
      body.set("player", playerName);
      body.set("caption", questPhotoCaption);
      const res = await fetch(`/api/party/valkyrie/quest-cards/${selectedQuestCard.id}/photos`, {
        method: "POST",
        body,
      });
      const json = await res.json();
      if (!res.ok || json.ok === false) throw new Error(json.error || "Could not attach photo");
      setQuestPhotoCaption("");
  ...
  const saveAdminCardCopy = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const json = await postJson<{ card: OracleCard }>("/api/party/valkyrie/admin/card-copy", {
        admin_token: adminToken,
        ...adminCopyDraft,
      });
      localStorage.setItem("valkyrie_party_admin", adminToken);
  ...
  const uploadAdminCardImage = useCallback(async (file: File | null) => {
    ...
    try {
      const body = new FormData();
      body.set("admin_token", adminToken);
      body.set("cardId", selectedAdminCardId);
      body.set("file", file);
      const res = await fetch("/api/party/valkyrie/admin/card-upload", {
        method: "POST",
        body,
      });
      const json = await res.json();
      if (!res.ok || json.ok === false) throw new Error(json.error || "Could not upload image");
      localStorage.setItem("valkyrie_party_admin", adminToken);
  ...
  return (
    <main style={{ minHeight: "100vh", background: `radial-gradient(circle at 18% 8%, rgba(255,176,0,0.28), transparent 28%), radial-gradient(circle at 82% 4%, rgba(255,77,46,0.24), transparent 26%), linear-gradient(180deg, ${PARTY_BG}, #220700 72%)`, color: PARTY_CREAM, fontFamily: "Georgia, serif", padding: "1.25rem" }}>
      ...
      <div style={{ width: "100%", maxWidth: 1080, margin: "0 auto", display: "grid", gap: "1rem" }}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.3fr) minmax(min(100%, 260px), 0.7fr)", gap: "1rem" }}>
          <Panel title="Party Flow">
            ...
            <p style={{ margin: "0 0 0.75rem", opacity: 0.78, lineHeight: 1.45 }}>
              This is a loose spa-day flow, not a strict schedule. We can interrupt the room for check-ins, invitations, treasure moments, and anything the Fire Bird rite needs.
            </p>
            <div style={{ display: "grid", gap: "0.75rem" }}>
          ...
          <Panel title="Shared Altar">
            <p style={{ margin: "0 0 0.7rem", opacity: 0.78 }}>
              {signups.length} players have joined. Public offerings, photos, replies, and "turn this into a card" actions belong in the altar space.
            </p>
            <p style={{ margin: 0, opacity: 0.72, lineHeight: 1.45 }}>
              The altar should open with context from your discoveries and tags, so it can offer the right moves instead of showing every composer everywhere.
            </p>
