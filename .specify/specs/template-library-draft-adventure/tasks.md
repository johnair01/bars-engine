...
- [x] **1.1** Add `AdventureTemplate` model to `prisma/schema.prisma`:
  - key (String, unique)
  - name (String)
  - description (String?)
...
- [x] **1.3** Create `scripts/seed-adventure-templates.ts`:
...
- [x] **2.1** Create `src/lib/template-library/index.ts`:
...
- [x] **2.2** For generateFromTemplate: ensure startNodeId is set on Adventure; create passages in slot order; use unique slug (e.g. `encounter-{timestamp}` or `encounter-{cuid}`)

## Phase 3: Admin UI

- [x] **3.1** Create `src/app/admin/templates/page.tsx`:
...
- [x] **3.2** Create `src/app/admin/templates/actions.ts`:
  - `generateFromTemplateAction(templateId, formData?)` — call service, redirect

- [x] **3.3** Edit `src/app/admin/adventures/[id]/page.tsx`:
  - When adventure.status === 'DRAFT': add "Promote to Active" button
  - Wire to promoteDraftToActive Server Action

- [x] **3.4** Add promoteDraftToActive to adventures actions (create or extend `src/app/admin/adventures/[id]/actions.ts`)

- [x] **3.5** Add "Templates" link: Admin nav or /admin/adventures page (e.g. "Create from template" or link to /admin/templates)

## Phase 4: Verification Quest

- [x] **4.1** Create verification quest `cert-template-library-v1`:
...
- [x] Generate from template → Adventure with 9 passages
- [x] Promote to Active works
- [x] Verification quest is seedable and completable
