...
| Topic | Decision |
|-------|----------|
| **Inventory surface** | **Vault is the single canonical personal inventory.** The Vault is already a deliberate **five-move room dashboard** (Wake·Open·Clean·Grow·Show → `/vault/charges`, `/vault/open-up`, `/vault/drafts`, `/vault/quests`, …); the fix is to **fold the `/bars` "Inspirations" flat list in as a canonical "All BARs" room and redirect `/bars` → the Vault** — **not** to flatten the move-rooms. A shared `getVaultInventory` read-model backs the new All-BARs room + lobby counts (reusing `listMyBars` semantics) and retires the 50-item `VAULT_SERVER_LIST_CAP` asymmetry with cursor pagination. The BAR **detail** page (`/bars/[id]`) stays. |
