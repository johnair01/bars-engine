...
- [x] **1.4** Document cache opportunities per ai-deftness-token-strategy
...
- [x] **2.5** Add API Contracts section to game-master-face-moves if it defines server actions (check EXPLORATION.md)

## Phase 3: Verification

- [x] **3.1** Run `npm run test:gm-agents` — backend auto-starts, all 6 agent APIs return valid JSON
- [x] **3.2** Invoke sage_consult("What is the BARS Engine?") — returns synthesis (via test:gm-agents)
- [x] **3.3** With OPENAI_API_KEY unset — invoke sage_consult, architect_draft — both return valid JSON (deterministic fallbacks)
- [ ] **3.4** Run `npm run build` and `npm run check` — pass (daemons.ts errors pre-existing; forge.ts fixed)
- [x] **3.5** Confirm all GM specs have Practice tag; plans/tasks exist where applicable

## Verification Summary

| Check | Status |
|-------|--------|
| MCP tools documented | done |
| Deterministic fallbacks verified | done |
| deftness_context in all agents | done |
| Practice tag on GM specs | done |
| plan/tasks for template-content-generation | done |
| Build + check pass | blocked by daemons.ts (pre-existing) |
